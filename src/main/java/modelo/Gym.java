...
public class Gym {
	...
	int colNumber = clients[0].toObjectVector().length;
	Object[][] datas = new Object[position][colNumber];
	for (int i = 0; i < datas.length; i++) {
		datas[i] = clients[i].toObjectVector();
	}
	return datas;
    }
	
    public Object[] toVector(String documentNumber) {
    	int colNumber = clients[0].toObjectVector().length;
	Object[] datas = new Object[colNumber];
	for (int i = 0; i < clients.length; i++) {
		if (documentNumber.equals(clients[i].getDocumentNumber())) {
			datas = clients[i].toObjectVector();
		}
	}
	return datas;
    }
